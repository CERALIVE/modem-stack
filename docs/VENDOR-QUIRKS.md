...
| **`AT!`-prefixed commands (`AT!BAND`, `AT!ENTERCND`) are a password-gated vendor surface.** | MM — the `sierra` plugin is a first-class plugin at `1.24.2`. CeraLive's own AT fence lists no `AT!` form: `providers/ufi-himi`'s static gate scans for `AT!` as a FORBIDDEN construct. | `unavailable` — no `AT!` command exists anywhere in this repository, by gate. |
